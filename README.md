...
1. **Hierarchical Multi-Label Intent Classification**: Enable primary and secondary intent tagging for multi-grievance tweets (e.g., flight delayed + baggage lost).
2. **Dense Domain Embeddings**: Replace sparse TF-IDF with fine-tuned domain sentence-transformers (`all-MiniLM-L6-v2`) to lift top-1 retrieval similarity above 0.60.
3. **Calibrated Per-Intent Thresholding**: Tune decision thresholds per category to lift escalation precision from 0.220 to 0.650 while maintaining 1.000 safety recall.
4. **Subword & Slang Normalization**: Preprocess Twitter abbreviations (`pls`, `cld`, `comped`, `AA123`).
5. **Multi-Rater Agreement Study**: Recruit two independent human evaluators using `data/golden/human_review_template.csv` to compute inter-human Cohen's $\kappa$ as an empirical ceiling.
...
├── REPORT.md                # Formal engineering evaluation report
├── decision_log.md          # 15 non-obvious design & implementation decisions
├── requirements.txt         # Minimal reproducible dependencies
├── .env.example             # Template environment variables
├── .gitignore
│
├── data/
│   ├── raw/                 # Raw Kaggle TWCS sample & ingestion scripts
│   ├── processed/           # 500 historical conversations KB (conversations.csv)
│   └── golden/              # 200 real customer tweets + audit template
│
├── src/
│   ├── config.py            # Central configuration & hyperparameters
│   ├── agent.py             # SupportAgent pipeline with safety-first escalation
│   ├── intents.py           # 10 data-derived intent taxonomy definitions
│   ├── intent_classifier.py # TF-IDF + LogisticRegression baseline
│   ├── embeddings.py        # Dense & sparse vector embedders
│   ├── retriever.py         # FAISS / NearestNeighbor historical retriever
│   ├── escalation.py        # Deterministic 4-rule safety guardrail engine
│   └── response_generator.py# Grounded response generator
│
├── evaluation/
│   ├── baseline_majority.py # Trivial majority baseline
│   ├── baseline_tfidf.py    # Simple TF-IDF baseline
│   ├── metrics.py           # Macro-F1, precision, recall, confusion matrix
│   ├── llm_judge.py         # 5-dimension Gemini LLM-as-a-Judge
│   └── human_agreement.py   # Pearson r & Quadratic Kappa
│
├── notebooks/
│   ├── 01_eda.py            # Exploratory data analysis on TWCS
│   ├── 01_intent_discovery.py # Unsupervised K-Means clustering script
│   └── 01_intent_discovery.ipynb # Jupyter notebook for intent discovery
│
├── scripts/
...
│   └── run_evaluation.py    # Automated end-to-end evaluation runner
│
├── results/
│   ├── results.json         # Complete benchmark metrics
│   ├── failures.csv         # 126 categorized real-world error cases
│   ├── judge_scores.csv     # Gemini LLM judge evaluation records
│   ├── llm_judge_results.csv# Itemized judge scores with reasoning
│   ├── confusion_agent.png  # Agent confusion matrix
│   └── confusion_tfidf.png  # TF-IDF baseline confusion matrix
│
└── app/
    └── streamlit_app.py     # Interactive support agent web interface
```

---

## 12. Technical Report & Decision Log
